...
final class NickSkinsCommands {
	...
	private static List<Target> players(CommandContext<ServerCommandSource> ctx) throws CommandSyntaxException {
		...
		for (ServerPlayerEntity p : players) targets.add(new Target(p.getUuid(), p.getGameProfile().name()));
		return targets;
	}
	...
	private static void checkNick(String nick) throws CommandSyntaxException {
		...
		// Same rule vanilla uses for player names: printable ASCII, no spaces. Anything else can
		// fail to encode and kick viewers.
		for (int i = 0; i < nick.length(); i++) {
			char c = nick.charAt(i);
			if (c <= 32 || c >= 127) {
				throw error("Nicknames can only use letters, numbers and symbols, with no spaces "
						+ "(Minecraft's rule for player names). Try " + nick.trim().replaceAll("\\s+", "_"));
			}
		}
		if (nick.indexOf('&') >= 0) {
	...
	private static int groupAdd(CommandContext<ServerCommandSource> ctx) throws CommandSyntaxException {
		...
		int added = 0;
		for (var entry : GameProfileArgumentType.getProfileArgument(ctx, "players")) {
			if (members.put(entry.id(), entry.name()) == null) added++;
		}
		storage().saveData();
