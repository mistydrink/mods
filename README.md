# Fabric server mods for Minecraft 1.21.11
...
[Fabric API](https://modrinth.com/mod/fabric-api) for 1.21.11 into the server's `mods/` folder.
LuckPerms support (fabric-permissions-api) is bundled inside each jar.

The projects use Yarn mappings and target 1.21.11 exactly. Other versions change some of the
...
| Command | Effect |
|---|---|
| `/nick set <targets> <name>` | Give everyone matched the same nickname, e.g. `/nick set @a[team=red] RedGuard` |
| `/nick numbered <targets> <prefix>` | Numbered nicknames: `Guard1`, `Guard2`, … (use `Guard_` for `Guard_1`) |
...
1. Open `.minecraft/versions/1.21.11/1.21.11.jar` as a zip.
...
- **Nickname rules.** Nicknames follow Minecraft's rules for real player names: at most **16 characters**, letters, numbers and symbols only, and **no spaces** (use `_`). They also can't be colored above heads.
